...
//通过js 往页面上面创建svg 元素.
d3.select("body").append("svg").attr("width",300)//往body里面append添加标签
...
![three_render](F:\baiduDown\10课   D3.js three.js\day_11\笔记\three_render-1531203593826.jpg)
...
var materials = [
    ...
    new THREE.MeshBasicMaterial({color: 'yellow'}),//正面
...
render();
function render(){//动画旋转立方体
    renderer.render(sence,carma);
    //调整几何体的位置
    cube.rotation.x+=0.01;
    cube.rotation.y+=0.01;
    cube.rotation.z+=0.01;
    //当你需要更新屏幕画面时就可以调用此方法[window下的方法]。在浏览器下次重绘前执行回调函数。回调的次数通常是每秒60次，但大多数浏览器通常匹配 W3C 所建议的刷新频率。在大多数浏览器里，当运行在后台标签页或者隐藏的<iframe> 里时，requestAnimationFrame() 会暂停调用以提升性能和电池寿命。
